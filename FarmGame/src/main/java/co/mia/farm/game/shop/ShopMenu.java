...
public class ShopMenu {
	...
	public void run() {
		...
		while (true) {
			...
			if (menu == 1) { // 구매
				boolean flag = is.productPrint(shopItems);
				if(flag) {
					...
					if (ans > 0 && ans < shopItems.size() + 1) {
						...
						if (cnt < 0) {
							...
							if (LoginMenu.loginCharacter.getUserMoney() >= purMoney) {
								LoginMenu.loginCharacter.setUserMoney(LoginMenu.loginCharacter.getUserMoney() - purMoney); // 유저 돈 뺏기
				...
				if (ans > 0 && ans < myItems.size() + 1) {
					...
					if(cnt > 0 && cnt <= item.getItemCnt()) {
						...
						LoginMenu.loginCharacter.setUserMoney(LoginMenu.loginCharacter.getUserMoney()+addMoney); //캐릭터 돈 더 주기
